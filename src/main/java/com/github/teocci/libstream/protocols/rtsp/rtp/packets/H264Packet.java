...
public class H264Packet extends BasePacket
{
    ...
    public H264Packet(Session session)
    {
        super(session);
    ...
    @Override
    public void createAndSendPacket(ByteBuffer buffer, BufferInfo videoInfo)
    {
        boolean interrupted = false;
        try {
            // We read a NAL units from ByteBuffer and we send them
            // NAL units are preceded with 0x00000001
            buffer.get(header, 0, 5);
            ts = videoInfo.presentationTimeUs * 1000L;
            int naluLength = videoInfo.size - buffer.position() + 1;
            int type = header[4] & 0x1F; // 00011111 = 0x1F

            if (type == 5) {
                this.buffer = socket.requestBuffer();
                socket.markNextPacket();
                socket.updateTimestamp(ts);
                System.arraycopy(stapA, 0, this.buffer, RTP_HEADER_LENGTH, stapA.length);
            ...
            if (naluLength <= MAX_PACKET_SIZE - RTP_HEADER_LENGTH - 2) {
                ...
                this.buffer = socket.requestBuffer();
                this.buffer[RTP_HEADER_LENGTH] = header[4];

                int cont = naluLength - 1;
                int bufferSize = videoInfo.size - buffer.position();
                int length = cont < bufferSize ? cont : bufferSize;
                buffer.get(this.buffer, RTP_HEADER_LENGTH + 1, length);
                ...
                while (sum < naluLength) {
                    this.buffer = socket.requestBuffer();
                    this.buffer[RTP_HEADER_LENGTH] = header[0];
                    this.buffer[RTP_HEADER_LENGTH + 1] = header[1];
                    socket.updateTimestamp(ts);
                    ...
                    buffer.get(this.buffer, RTP_HEADER_LENGTH + 2, length);
                    if (length < 0) {
                        return;
                    }
                    ...
                    if (sum >= naluLength) {
                        // End bit on
                        // 01000000 = 0x40
                        this.buffer[RTP_HEADER_LENGTH + 1] += 0x40;
                        socket.markNextPacket();
                    }
